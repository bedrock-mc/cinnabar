...
func TestGeneratedTraceMatchesPinnedFixtureExactly(t *testing.T) {
	want, err := os.ReadFile("../../crates/sim/fixtures/bedsim-v0.1.4-liquid.jsonl")
	if err != nil {
		t.Fatal(err)
	}
	...
	if !bytes.Equal(first.Bytes(), second.Bytes()) {
		t.Fatal("two v0.1.4 liquid generations differ")
	}
...
func TestLiquidProvenanceBindsPinnedModuleGeneratorAndOutput(t *testing.T) {
	...
	bytes, err := os.ReadFile("../../crates/sim/fixtures/bedsim-v0.1.4-liquid.provenance.json")
	if err != nil {
		t.Fatal(err)
	}
	var got provenance
	...
	if got.Module != "github.com/oomph-ac/bedsim" || got.Version != "v0.1.4" || got.SourceCommit != "b55c95016bb53c3df3b13e9a5cd8cbbcacabbe28" || got.ModuleSum != "h1:oDfPiVgskqWnh9slic8Avdp+/Kd0NKWEJ2z2Ejghdq0=" || got.Generator != "tools/bedsimtrace-v0.1.4" || got.GeneratorCommand != "GOWORK=off go run ." {
		t.Fatalf("incomplete v0.1.4 provenance: %#v", got)
	}
	...
	assertFileHash(t, "../../crates/sim/fixtures/bedsim-v0.1.4-liquid.jsonl", got.SHA256)
}
