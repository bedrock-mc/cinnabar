    ...
    let mut visuals = records
        ...
        .map(|record| {
            if record.flags.contains(BlockFlags::AIR) {
                ...
                || diagnostic_ids.contains(&record.sequential_id)
            {
...
#[test]
fn gallery_inventory_has_exact_65_page_shape() {
    ...
    assert_eq!(inventory.pages.len(), 65);
    assert!(
        inventory.pages[..64]
            .iter()
            .all(|page| page.targets.len() == GALLERY_PAGE_CAPACITY)
    );
    assert_eq!(inventory.pages[64].targets.len(), 146);
    assert_eq!(inventory.pages[0].first_sequential_id, 0);
    assert_eq!(inventory.pages[64].last_sequential_id, 16_912);
}
...
fn gallery_inventory_contains_every_sequential_id_once_and_in_order() {
    ...
    let records = read_registry(&registry).unwrap();
...
fn current_gallery_inventory_is_non_accepting_with_2031_provisional_fallbacks() {
    ...
    assert!(!inventory.accepting);
    assert_eq!(inventory.diagnostic_targets, 0);
    assert_eq!(inventory.fallback_targets, 2_031);
    assert_eq!(
        inventory
            .pages
            .iter()
