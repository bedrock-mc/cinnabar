...
pub struct DiagnosticQuadTracker {
    by_sub_chunk: BTreeMap<SubChunkKey, DiagnosticGeometrySummary>,
...
impl DiagnosticQuadTracker {
    pub fn upsert(&mut self, key: SubChunkKey, summary: DiagnosticGeometrySummary) {
        ...
        if let Some(previous) = self.by_sub_chunk.remove(&key) {
            self.subtract_summary(&previous);
        }
        self.add_summary(&summary);
        self.by_sub_chunk.insert(key, summary);
    ...
    fn add_summary(&mut self, summary: &DiagnosticGeometrySummary) {
        ...
        for count in summary.entries() {
            ...
            } else if self.totals.len() < MAX_TRACKED_DIAGNOSTIC_IDENTITIES {
    ...
    fn subtract_summary(&mut self, summary: &DiagnosticGeometrySummary) {
        ...
        for count in summary.entries() {
            ...
            } else if !self.totals.contains_key(&key) {
                self.explicit_omitted_identity_count =
