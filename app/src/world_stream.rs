...
struct MeshCompletion {
    ...
    revision: u64,
    source: Arc<SubChunk>,
    biome_sources: BiomeNeighbourhood,
    biome: PackedBiomeRecord,
    tint_identity: ChunkBiomeTintIdentity,
    mesh: ChunkMesh,
...
impl LightBlockSnapshot {
    fn resolve_palette_light(&mut self) {
...
fn uniform_known_air_light(job: &PreparedLightJob) -> Option<(SubChunkLight, DirectSkyMask)> {
    if !matches!(
...
fn light_boundary_position(key: SubChunkKey, offset: [i32; 3], a: u8, b: u8) -> Option<BlockPos> {
...
struct MeshSnapshot {
    center: Arc<SubChunk>,
    biomes: BiomeNeighbourhood,
    adjacent: [Option<Arc<SubChunk>>; 27],
    light_halo: MeshLightHalo,
}

type BiomeNeighbourhood = [Option<Arc<BiomeStorage>>; BIOME_NEIGHBOUR_SLOT_COUNT];
...
impl WorldStream {
    ...
    /// Forces a new generation for every allocation in an already presented, frozen manifest.
    ///
    /// The manifest is rejected unless it is non-empty, contains unique resident keys, and
    /// exactly names the generation last acknowledged by the world stream for every key. This
    /// keeps the acceptance remesh tied to the complete published allocation cohort without
    /// manufacturing removal jobs for known-air or packed-empty subchunks.
    ...
    fn mark_changed_sources_with_mesh_dirty(
        &mut self,
        sources: impl IntoIterator<Item = SubChunkKey>,
    ...
    fn remove_light_waiters_for(&mut self, key: SubChunkKey) {
        self.light_waiters.remove(&key);
        self.remove_light_waiter_target(key);
    }

    fn remove_light_waiter_target(&mut self, key: SubChunkKey) {
    ...
    fn prior_light_may_seed(&self, target: SubChunkKey, neighbour: SubChunkKey) -> bool {
    ...
    fn light_dispatch_ready(&self, key: SubChunkKey) -> bool {
        if key.dimension != 0 {
            return true;
        }
    ...
    fn known_air_has_vertical_direct_sky(&self, key: SubChunkKey) -> bool {
    ...
    fn biome_neighbourhood(&self, key: SubChunkKey) -> BiomeNeighbourhood {
...
fn is_uniform_direct_sky(light: &SubChunkLight, direct: &DirectSkyMask) -> bool {
...
mod tests {
    ...
    mod light_scheduler {
        ...
        use render::{
            ...
            RenderViewCohort, VisibilityDiagnosticsInput,
        };
        use world::{
        ...
        fn release_full_view_publication_completes_within_two_seconds() {
            ...
            let biome_tints = ChunkBiomeTints::from_resolved_with_identity(
                ...
                stream.biome_tint_identity(),
            );
            let mut app = App::new();
            app.insert_resource(ChunkTextureAssets::new(Arc::clone(&stream.runtime_assets)))
            ...
            app.sub_app_mut(RenderApp)
                .insert_resource(VisibilityDiagnosticsInput::new(false));
            app.update();
            ...
            let witnesses = (-16..=12)
                ...
                .flat_map(|x| {
                    (-16..=12)
                        .step_by(4)
                        .map(move |z| SubChunkKey::new(0, x, -4, z))
                })
                .collect::<BTreeSet<_>>();
            assert_eq!(witnesses.len(), POPULATED_WITNESSES);
            ...
            // Keep the complete known-air solve as the fixture, then turn a
            // spaced in-cohort grid into deterministic non-empty mesh witnesses.
            // Their current light identities are installed explicitly so the
            // workers exercise the same halo validation as live publication.
            let mut witness_generations = BTreeMap::new();
            for &populated in &witnesses {
                let decoded = DecodedLevelChunk::decode(
                    populated.y,
                    1,
                    include_bytes!("../../crates/world/fixtures/uniform_non_air.bin"),
                ...
                stream.store.commit_level_chunk(populated.chunk(), decoded);
                install_current_light(&mut stream, populated, 0, 15, true);
                witness_generations.insert(
                    populated,
                    stream.mark_dirty_exact(populated, Instant::now()),
                );
            }
            assert_eq!(stream.resident.len(), VIEW_SUB_CHUNKS);
            assert_eq!(
                stream.known_air.len(),
                VIEW_SUB_CHUNKS - POPULATED_WITNESSES
            );
            ...
            let mut gpu_acknowledged_witnesses = BTreeSet::new();
            let mut applied_witnesses = BTreeSet::new();
            let mut idle_render_updates = 0_usize;
            ...
            loop {
                ...
                for acknowledgement in acknowledgements.drain() {
                    ...
                    if acknowledgement.uploaded_bytes != 0
                        && witnesses.contains(&acknowledgement.key)
                    {
                        gpu_acknowledged_witnesses.insert(acknowledgement.key);
                    }
                ...
                applied_witnesses.extend(
                    app.world_mut()
                        .query::<&ChunkRenderInstance>()
                        .iter(app.world())
                        .filter(|instance| instance.quad_count() != 0)
                        .map(ChunkRenderInstance::key)
                        .filter(|key| witnesses.contains(key)),
                );
                ...
                if !progressed && !stream.in_flight.is_empty() {
                    ...
                    idle_render_updates += 1;
                    assert!(
                        idle_render_updates <= 8,
                        "production GPU acknowledgement made no progress in eight render frames"
                    );
                    app.update();
                } else {
                    idle_render_updates = 0;
                }
            }
            ...
            let witness_expectation = app
                ...
                .expect("witnesses belong to the publication cohort");
            ...
            assert!(
                ...
                "bounded extraction used {upload_updates} updates, outside the minimum plus one tail per async populated mesh"
            );
            ...
            assert_eq!(gpu_acknowledged_witnesses, witnesses);
            assert_eq!(applied_witnesses, witnesses);
            assert_eq!(
    ...
    fn request_block_entity_event_with_biome(
        chunk_x: i32,
        biome_id: i32,
        nbt: Vec<u8>,
    ) -> WorldEvent {
    ...
    fn inline_biome_replacement_dirties_diagonal_cross_chunk_blend_dependents() {
        ...
        let block_payload = include_bytes!("../../crates/world/fixtures/uniform_non_air.bin");
