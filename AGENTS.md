...
- The user-approved compact Java-style bottom-left chat layout is the only current presentation exception. Its text semantics, Bedrock formatting, focus/input behavior, bounds, and server-authored content must still be correct; this exception does not authorize Java-style changes elsewhere.
- Never expose raw protocol structures, JSON/rawtext envelopes, placeholder geometry, duplicate HUD elements, numeric stand-ins, or debug-only presentation on a production player-facing surface.
