...
The app actor store keeps the normalized skin status beside the bounded player profile and enforces a cumulative retained-byte cap across incremental roster packets. It exposes a deterministic, runtime-ID-ordered list of player render sources after excluding the local runtime ID. A render-owned scene keeps at most 128 remote-player histories with at most two timed poses per actor. Samples are evaluated 100 ms behind the current render time, clamped at the ends, with shortest-path angle interpolation. A teleport replaces both endpoints once; repeated publication of the same sticky teleport state does not keep collapsing the next interpolation interval. This state is driven only by actor packets and `Time<Real>`; free-camera and local movement state are not inputs.
...
Focused tests cover protocol skin normalization and bounds, cumulative store retention, profile joining and local-player exclusion, bounded histories, delayed position/angle interpolation, one-shot teleport snapping, stable truncation, standard biped cuboid/UV descriptors, skin normalization/default provenance, plugin idempotence, WGSL parsing, pipeline specialization, and no-op-backend binding-layout validation. Strict formatting and workspace Clippy run before the local commit. Hardware-backend render-pipeline creation remains part of live visual acceptance, not an automated claim of this tranche.
