...
For `required_cohort_identity`, first add a behavior-neutral raw `publisher_radius_blocks` witness and rerun the Lunar diagnostic. Once the raw value and stage counts prove the classification, preserve exact block-radius geometry in `ViewCohort`: raw 120 produces 177 columns, raw 128 produces 197, and raw 256 preserves 797. Cover negative/unaligned centers, exclude the exact 20-position outer annulus introduced by rounding 120 blocks up to eight chunks, and prove 177 request-mode LevelChunk announcements complete that Lunar cohort while any actual member missing fails. Retain the ceiling chunk radius only for bounded active-retention calculations. Replace acceptance checks that compare the server-derived publisher radius with the client-requested radius: WORLD_READY, mutation, teleport, and remesh gates must bind to the exact committed raw block-radius cohort hash/count and separately retain the requested/received chunk-radius witness. Add a regression with requested radius 16 plus server raw radius 120 that passes only after all 177 exact members and the downstream presentation identities complete. Do not add `RequestClass` or change retry ordering in this branch.
