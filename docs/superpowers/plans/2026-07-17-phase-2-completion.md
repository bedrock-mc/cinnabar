...
**Files:**
- Required-cohort-identity branch only: modify `crates/client-world/src/stream/model.rs`, `crates/client-world/src/stream/diagnostics.rs`, `crates/client-world/src/stream/tests/cases_03.rs`, `crates/client-world/src/stream/tests/cases_05.rs`; integration handoff adds the raw-radius witness to `app/src/runtime/phase2_evidence.rs` and the remote manifest and corrects the exact-cohort acceptance owners in `app/src/acceptance/mutation.rs`, `app/src/acceptance/teleport.rs`, `app/src/acceptance/remesh.rs`, `app/src/acceptance/world_ready.rs`, and their focused tests
...
For `required_cohort_identity`, retain the behavior-neutral raw `publisher_radius_blocks` witness, but never infer a universal enumerable cohort from it: the protocol radius is a publisher-view envelope, and servers use different membership rules. Build each required cohort from the unique FIFO-committed request-mode `LevelChunk` coordinates in one publisher epoch, with a deterministic count/hash. A changed publisher identity, session, or dimension starts a fresh epoch; a later unique announcement expands the current epoch and invalidates readiness plus every frozen presentation identity until the expanded cohort completes and is presented. Keep the ceiling chunk radius only for bounded active retention. Lunar raw 128 is proven complete at 177 announcements; the attributable Dragonfly round-distance classifier is `distance < r - 0.5`, giving 177 columns at raw 128 and 749 at raw 256, while raw 120 => 177 is explicitly a compatibility policy because Dragonfly cannot emit it. These classifier counts are diagnostic/lower-bound fixtures, never protocol truth. WORLD_READY, mutation, teleport, and remesh gates must bind to the committed publisher epoch plus actual required count/hash and separately retain the raw publisher and requested/received radius witnesses. Regressions must prove raw 128 with all 177 announced members passes only after downstream presentation completes, any one incomplete member fails, a later outer 178th announcement invalidates the old proof, epoch changes reset membership, and every presentation-stage identity carries the same session/epoch/count/hash. Do not add `RequestClass` or change retry ordering in this branch.
