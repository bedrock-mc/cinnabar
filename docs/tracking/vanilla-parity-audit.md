...
| VPA-106 | Proven mismatch | Non-finite remote movement can poison retained actor pose until a later packet; unknown equipment windows can overwrite the right hand. | `crates/protocol/src/world.rs`, `crates/client-world/src/actor_store`, `crates/client-world/src/item.rs` | Skip only invalid fields/events and retain the prior usable actor/equipment state. |
