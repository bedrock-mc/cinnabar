...
The cache publication surface is part of the strict `PHASE2_PUBLICATION` schema. The app forwards live Play-session enablement and aggregate counters into `ClientWorld`, publishes only secret-safe totals, and uses a bounded non-cancellable final control flush before network failure or ordinary stop. The acceptance parser requires the exact cache schema, exact Boolean enablement, unsigned integral counters, `hits + misses = hashes_classified`, zero counters while disabled, stable enablement, and non-regressing cumulative counters across a diagnostic sequence. Production currently keeps the client blob cache disabled while resolver hardening remains open. A Lunar run may therefore prove current runtime compatibility and ordinary-payload publication only when both client publication and independent proxy boundary evidence agree that caching is disabled, all cache counters are zero, the ordinary route is attributable, and rejected/pending state is zero. Such a manifest is explicitly marked `client_blob_cache_performance_gate_deferred`; it is not evidence that the cache-backed performance gate is complete. The eventual Lunar cache-backed gate still requires enabled cache-backed hash activity with zero rejected blobs and zero pending transactions/bytes. Zeqa continues to record either a coherent `ordinary_payload` or `cache_backed` route with the same clean terminal state.
