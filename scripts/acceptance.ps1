...
try {
    ...
    if ($VisualFixturePose -ne 'None' -or $LeafForestBaseline -or $LeafForestFullView) {
        # Snapshot the freshly copied world before the measured BDS process
        # opens and locks level.dat. A clean BDS source has no world yet, so
        # bootstrap it once on temporary reserved ports, stop it cleanly, and
        # bind the resulting unlocked world identity before the measured run.
