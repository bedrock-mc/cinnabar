...
fn generic_metadata_flag(actor: &ActorSnapshot, bit: u32) -> bool {
...
fn extended_sleeping_flag(actor: &ActorSnapshot) -> bool {
    matches!(actor.metadata.get(&92), Some(ActorMetadataValue::FlagsExtended(flags)) if flags & (1 << 11) != 0)
}

fn compose_pose(bones: &[RuntimeBone], local: &[LocalDelta]) -> Option<Vec<BoneTransform>> {
...
fn quat_from_euler(rotation: [f32; 3]) -> [f32; 4] {
...
fn pop(stack: &mut Vec<f32>) -> Result<f32, EvalError> {
