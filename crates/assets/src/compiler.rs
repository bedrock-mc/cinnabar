...
fn is_selector_alias_cube_name(name: &str) -> bool {
    SELECTOR_ALIAS_CUBE_NAMES.binary_search(&name).is_ok()
}

fn exact_tagged_string(value: &serde_json::Value) -> Option<&str> {
...
fn exact_tagged_byte(value: &serde_json::Value, maximum: u8) -> Option<u8> {
    let object = value.as_object()?;
...
fn exact_selector_alias_cube_state(record: &RegistryRecord) -> Option<u32> {
    let state =
...
fn is_selector_alias_cube_record(record: &RegistryRecord) -> bool {
...
fn selector_alias_cube_inventory_is_exact(records: &[RegistryRecord]) -> bool {
...
fn selector_alias_cube_material_descriptors(
    pack: &PackSources,
    record: &RegistryRecord,
...
fn is_cake_name(name: &str) -> bool {
    name == "minecraft:cake"
}

fn exact_cake_bite(record: &RegistryRecord) -> Option<u32> {
...
fn cake_collision_is_exact(record: &RegistryRecord, bite: u32) -> bool {
    let min_x = [
...
fn is_cake_record(record: &RegistryRecord) -> bool {
...
fn cake_inventory_is_exact(records: &[RegistryRecord]) -> bool {
    let selected = records
        .iter()
...
fn cake_material_descriptors(pack: &PackSources) -> Option<[(Descriptor, Box<str>); 4]> {
...
fn cake_source_alpha_is_exact(root: &Path, pack: &PackSources) -> bool {
...
const fn is_sign(record: &RegistryRecord) -> bool {
...
fn compile_visuals(
    ...
    for record in ordered_records {
            ...
            && (!admit_selector_alias_cubes || !is_selector_alias_cube_record(record))
        {
            ...
            let materials =
                selector_alias_cube_material_descriptors(pack, record).map(|descriptors| {
                    descriptors
                        .map(|(descriptor, _)| material_by_descriptor.get(&descriptor).copied())
                });
            if let Some(
                [
                    Some(west),
            ...
            || (is_copper_grate_name(&record.name) && !is_copper_grate(record))
        {
            ...
            if let (Some(material), Some(state)) = (material, sign_state(record)) {
                ...
                let template = if let Some(&template) = sign_template_by_key.get(&key) {
                    template
                } else {
                    let quads = sign_quads(material, state);
                    let template =
                        push_model_template(quads, 0, &mut model_templates, &mut model_quads)?;
...
fn sign_state(record: &RegistryRecord) -> Option<SignState> {
...
fn sign_quads(material: u32, state: SignState) -> Vec<ModelQuad> {
    let materials = [material; 6];
    match state {
        SignState::Standing { rotation } => {
...
fn wall_sign_quads(materials: [u32; 6], facing: u8) -> [ModelQuad; 6] {
...
fn hanging_wall_sign_quads(materials: [u32; 6], facing: u8) -> Vec<ModelQuad> {
    let mut quads = Vec::with_capacity(12);
...
fn rotate_sign_quads(mut quads: Vec<ModelQuad>, rotation: u8) -> Vec<ModelQuad> {
...
const fn model_quad_face_from_id(face: u32) -> Option<BlockFace> {
    match face {
...
const fn model_quad_face_id(face: BlockFace) -> u32 {
    match face {
        BlockFace::Down => 1,
        BlockFace::Up => 2,
        BlockFace::West => 3,
