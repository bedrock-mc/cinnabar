...
impl ScoreboardStore {
    ...
    /// Monotonic presentation revision. Ignored or rejected events do not advance it.
    ...
    /// Projects at most `maximum_rows` matching rows in the objective's exact
    /// score/entry-id order. Only the selected rows are cloned and materialized.
