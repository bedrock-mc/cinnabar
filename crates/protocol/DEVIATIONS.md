...
## Open findings before the Phase 0.8 zero-error gate

- A diagnostic read beyond the Task 0.5 StartGame boundary found generated
  `AvailableCommands` drift: the live 356,513-byte body failed with a declared
  string length of 2,194 and 942 bytes remaining. Task 0.5 does not consume this
  post-login packet; it must be localised, fixture-tested, and resolved before
  the 15-minute Phase 0.8 acceptance run.
- Static comparison found `MaterialReducer` modelled as one output in Valentine,
  while gophertunnel writes a VarUInt output count followed by that many
  `(network_id, count)` pairs. The current BDS CraftingData packet had no
  triggering reducer entry, but conformance coverage is required before the
  same Phase 0.8 gate.
